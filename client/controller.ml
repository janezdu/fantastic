...
(*let rec repl current_player (w: world): unit =
  let c = read_line ()  in  
    let new_s = (try let temp = (do_commadn c) in print_endline "Ok! "; temp with
        | _ -> (print_endline 
        "I'm sorry, I don't quite understand your command, please try again";  
        s)) in 

    try (match parse_c c with
    | Go _->  repl new_s
    | Drop i-> repl new_s
    | Take i -> repl new_s
    | Look -> 
        print_description new_s current_r items_in_room; repl new_s
    | Quit -> ()
    | Inventory -> (print_endline "------------Your Inventory------------"; 
        print_str_list s.curr_inventory; print_endline "------------------"); 
        repl new_s
    | Score -> print_endline (string_of_int (score s)); repl new_s
    | Turns -> print_endline (string_of_int (s.n_turns)) ; repl new_s) with
    | _ -> repl new_s
